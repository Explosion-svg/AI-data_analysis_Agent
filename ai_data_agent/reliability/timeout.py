...
核心机制（asyncio.wait_for）：
  ...
  本模块将 asyncio.TimeoutError 包装为自定义 AsyncTimeoutError，
  附加 name 和 timeout 属性，方便日志和上层异常处理识别。
  （P4-7：类名从 TimeoutError 改为 AsyncTimeoutError，
   避免遮蔽内置 TimeoutError——Python 3.11+ 中 asyncio.TimeoutError
   与内置 TimeoutError 同体，同名类会导致错误归属错乱。）
...
class AsyncTimeoutError(asyncio.TimeoutError):
    """
    超时异常，携带操作名称和超时时长信息（P4-7：重命名避免遮蔽内置 TimeoutError）。
...
async def run_with_timeout(
    ...
    """
    在 timeout 秒内执行协程，超时则抛出 AsyncTimeoutError。

    包装 asyncio.wait_for 的原因：
    1. 统一日志：超时时自动记录 WARNING 日志（含操作名和超时时长）
    2. 异常封装：将 asyncio.TimeoutError 包装为更有信息量的 AsyncTimeoutError
    ...
    Raises:
        AsyncTimeoutError: 协程在 timeout 秒内未完成
    ...
    try:
        ...
        raise AsyncTimeoutError(name, timeout)
